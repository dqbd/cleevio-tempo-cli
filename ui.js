...
const parseDate = str => typeof str === 'string' ? Date.parse(`${str}Z`) : null
...
function Tracker ({ tracker, selected }) {
	...
	return (
		<Box key={tracker.id}>
			...
			<Box marginRight={2} width={9} justifyContent="center" alignItems="flex-start">
				<Color
					keyword={(tracker.isPlaying ? "green" : "white")}
				>
			...
			{trackers !== false && (
				...
				>
					{trackers.map((tracker, index) => {
						return (
							<Tracker key={tracker.id} selected={index === selected} tracker={tracker} />
						)
					})}
				</Box>
			)}
